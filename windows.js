...
const isOn = () => execa.shell('ping google.com -n 1 -w 1000').then(() => true).catch(() => false);
...
const toggleDevice = turnOn => device().then(deviceInterface => execa.shell(`netsh interface set interface "${deviceInterface}" ${(turnOn) ? 'enabled' : 'disabled'}`).catch(() => {throw new Error('You don\'t have any WI-FI device or your run this without administrator privilegies!')}));
